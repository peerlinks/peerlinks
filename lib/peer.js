...
export default class Peer {
  ...
  async onQuery(packet) {
    Channel.checkId(packet.channelId.length, 'Invalid channelId in Query');
    if (packet.cursor === 'hash') {
      Message.checkHash(packet.hash, 'Invalid cursor.hash in Query');
    }

    const channel = this.channels.find((channel) => {
      return channel.id.equals(packet.channelId);
    });
    ...
    const cursor = packet.cursor === 'hash' ? { hash: packet.hash } :
      { height: packet.height };

    const result = await channel.query(
      cursor, packet.isBackward, packet.limit);
    this.debug('query for channel.id=%s count=%d', channel.debugId,
      result.abbreviatedMessages.length);
  ...
  async onQueryResponse(packet) {
    Channel.checkId(packet.channelId.length,
      'Invalid channelId in QueryResponse');
