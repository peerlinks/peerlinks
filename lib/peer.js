...
export default class Peer {
  ...
  constructor(options = {}) {
    ...
    this.syncWaitList = new WaitList();
  ...
  waitForSync(timeout) {
    ...
    return this.syncWaitList.waitFor(`${this.debugId} sync`, timeout);
  }
  ...
  synchronize(channel) {
    ...
    this.getSyncAgent(channel).synchronize().then(() => {
      if (this.destroyed) {
        return;
      }

      this.debug('channel.id=%s sync complete', channel.debugId);
      this.syncWaitList.resolve(`${this.debugId} sync`, channel);
    }).catch((e) => {
