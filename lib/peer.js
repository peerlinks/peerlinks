...
export default class Peer {
  ...
  async onSyncRequest(packet) {
    const tbs = packet.tbs;
    const seq = tbs.seq;

    Channel.checkId(tbs.channelId, 'Invalid channelId in SyncRequest');
    const channel = this.getChannel(tbs.channelId);
    ...
    if (!channel.isFeed) {
      const result = this.getSyncRequestKey(channel, packet);
      if (!result) {
    ...
    try {
      content = PSyncRequest.Content.decode(
        channel.decrypt(tbs.box, tbs.nonce));
    } catch (e) {
    ...
    const box = EphemeralBox.encryptFor(tbs.responsePubKey, encoded,
      { sodium: this.sodium });
  ...
  getSyncRequestKey(channel, packet) {
    const sodium = this.sodium;

    const chain = Chain.deserialize(packet.tbs.chain, { sodium });
    ...
    const isValid = Identity.verify({
      ...
      tbs: PSyncRequest.TBS.encode(packet.tbs).finish(),
      publicKey: leafKey,
    });
    if (!isValid) {
      return false;
