...
import { compareDistance, BanError, EphemeralBox } from './utils';
...
export default class Peer {
  ...
  async ready() {
    ...
    let hello;
    try {
      hello = PHello.decode(first);
    } catch (err) {
      this.debug('hello decoding err=%j', err.stack);
      throw new BanError('Failed to decode Hello');
    }
    ...
    let shake;
    try {
      shake = PShake.decode(second);
    } catch (err) {
      this.debug('shake decoding err=%j', err.stack);
      throw new BanError('Failed to decode Shake');
    }
  ...
  async loop() {
    ...
    for (;;) {
      ...
      let packet;
      try {
        packet = PPacket.decode(data);
      } catch (err) {
        this.debug('packet decoding err=%j', err.stack);
        throw new BanError('Failed to decode Packet');
      }
  ...
  async onSyncRequest(packet) {
    ...
    let content;
    try {
      content = PSyncRequest.Content.decode(
        channel.decrypt(packet.box, packet.nonce));
    } catch (err) {
      this.debug('sync request content decoding err=%j', err.stack);
      throw new BanError('Failed to SyncRequest.Content');
    }

    if (!channel.isFeed) {
      const result = this.getSyncRequestKey(channel, content);
      if (!result) {
        throw new BanError('Expected valid chain in SyncRequest');
      }

      const { leafKey, chain } = result;
    ...
    let response;
    switch (content.tbs.content) {
    case 'query':
      response = await this.onQuery(channel, seq, content.tbs.query);
      break;
    case 'bulk':
      response = await this.onBulk(channel, seq, content.tbs.bulk);
      break;
    default:
      throw new BanError(
        'Unsupported sync request type: ' + content.tbs.content);
    }
    ...
    const box = EphemeralBox.encryptFor(content.tbs.responsePubKey, encoded,
      { sodium: this.sodium });
  ...
  getSyncRequestKey(channel, content) {
    const sodium = this.sodium;

    const chain = Chain.deserialize(content.tbs.chain, { sodium });
    ...
    const isValid = Identity.verify({
      ...
      tbs: PSyncRequest.TBS.encode(content.tbs).finish(),
      publicKey: leafKey,
    });
    if (!isValid) {
      return false;
  ...
  async onSyncResponse(packet) {
    ...
    const box = agent.getRequestBox(seq);
    if (!box) {
    ...
    let content;
    try {
      content = PSyncResponse.Content.decode(box.decrypt(packet.box));
    } catch (err) {
      this.debug('sync response content decoding err=%j', err.stack);
      throw new BanError('Failed to SyncResponse.Content');
    }
