...
export default class Protocol {
  ...
  async channelFromInvite(invite, identity, options) {
    ...
    const channel = await Channel.fromInvite(invite, identity, options);
  ...
  async connect(socket) {
    ...
    try {
      ...
      for (const existing of this.peers) {
        ...
        // Let one peer to decide for another to avoid both peers killing
        // each others connections
        if (Buffer.compare(this.id, peer.remoteId) > 0) {
          this.debug('duplicate peer.id=%s closing', peer.debugId);
          await peer.destroy('Duplicate peer');
          return false;
        } else {
          this.debug('duplicate peer.id=%s ignoring', peer.debugId);
        }
      }
