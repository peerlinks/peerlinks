...
export default class Protocol {
  ...
  async load() {
    ...
    for (const id of channelIds) {
      ...
      const channel = await Channel.deserializeData(serialized, {
    ...
    for (const id of identityNames) {
      ...
      const identity = Identity.deserializeData(serialized);
  ...
  async saveIdentity(id) {
    ...
    await this.storage.storeEntity(
      'identity', id.publicKey.toString('hex'), id.serializeData());
  }
