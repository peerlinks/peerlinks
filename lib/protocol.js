...
export default class Protocol {
  ...
  constructor({ sodium, storage, passphrase } = {}) {
    ...
    this.debugId = this.publicKey.toString('hex').slice(0, 8);
  ...
  async channelFromInvite(invite, identity, options) {
    ...
    const channel = await Channel.fromInvite(invite,
      Object.assign({}, options, { identity }));
   ...
   *   `requestId`
   */
  resolveInvite(requestId, encryptedInvite) {
    return this.inviteWaitList.resolve(
      requestId.toString('hex'), encryptedInvite);
  }
  ...
  async connect(socket) {
    ...
    try {
      ...
      // Self-connect
      if (this.publicKey.equals(peer.remoteId)) {
        this.debug('self-connect');
        await peer.destroy('Self connect');
  ...
  async close() {
    ...
    this.inviteWaitList.close(new Error('Closed'));
    this.peerWaitList.close(new Error('Closed'));
  }
