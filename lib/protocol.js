...
import { isSameChainMap } from './utils';
...
export default class Protocol {
  ...
  async load() {
    ...
    for (const id of channelIds) {
      ...
      await this.addChannel(channel, false);
   ...
   * @param {Number} [timeout] - Optional timeout value in milliseconds
   * @returns {WaitListEntry} An object with `.promise` property and `.cancel`
   *     method. The `promise` resolves to `true` on chain map update and
   *     `false` otherwise.
   */
  waitForChainMapUpdate(timeout) {
    ...
    const entries = [
      // Wait for new chains
      this.waitList.waitFor('chain', timeout),
      // ...or for leaving peers
      this.waitList.waitFor('peer leave', timeout),
    ];
    ...
    const promise = Promise.race(entries).then(() => {
      return this.cachedChainMap !== this.computeChainMap();
    }).finally(() => {
      cancel();
    });
  ...
  computeChainMap() {
    ...
    for (const chains of result.values()) {
      chains.sort(Chain.compare);
    }

    // Compare to the past chain map
    if (this.cachedChainMap && isSameChainMap(this.cachedChainMap, result)) {
      return this.cachedChainMap;
    }

    // Changes!
    this.cachedChainMap = result;
