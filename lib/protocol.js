...
export default class Protocol {
  ...
  waitForInvite(requestId, timeout) {
    ...
    return this.inviteWaitList.waitFor(
  ...
  async connect(socket) {
    ...
    const syncLoop = async () => {
      for (;;) {
        let channel;
        try {
          channel = await peer.waitForSync().promise;
        } catch (e) {
          return;
        }
    ...
    try {
      ...
      this.peers.add(peer);
      this.peerWaitList.resolve(
        `peer ${peer.remoteId.toString('hex')}`, peer);
      this.peerWaitList.resolve('peer', peer);
  ...
  waitForPeer(peerId, timeout) {
    ...
    if (!peerId) {
      return this.peerWaitList.waitFor('peer', timeout);
    }
    ...
    return this.peerWaitList.waitFor(
  ...
  async close() {
    ...
    this.inviteWaitList.close();
    this.peerWaitList.close();
  }
