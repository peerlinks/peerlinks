...
export default class Protocol {
  ...
  constructor({ storage } = {}) {
    ...
    debug('local peer id=%s', this.id.toString('hex').slice(0, 8));
  }
  ...
  async createIdentity(name, createChannel = true) {
    ...
    debug('created id.name=%s', identity.name);
    return identity;
  }
  ...
  onNewMessage(channel) {
    debug('broadcasting new message on channel.id=%s', channel.debugId);
    for (const peer of this.peers) {
  ...
  async connect(socket) {
    debug('connecting through socket');
    ...
    try {
      ...
      for (const existing in this.peers) {
  ...
  waitForPeer(peerId) {
    for (const existing of this.peers) {
      if (existing.remoteId.equals(peerId)) {
        debug('found existing peer.id=%s', existing.debugId);
