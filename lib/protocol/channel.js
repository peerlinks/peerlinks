...
export default class Channel {
  ...
  static async fromIdentity(identity, options) {
    const channel = new Channel(Object.assign({}, options, {
  ...
  static async fromInvite(invite, options) {
    ...
    await channel.receive(Message.deserialize(invite.channelRoot, { sodium }));
  ...
  async post(body, identity, { timestamp = now() } = {}) {
    ...
    // Find the leaves that are not in the future
    leaves = await this.findPastLeaves(leaves, timestamp);
  ...
  async receive(message) {
    ...
    if (message.parents.length > MAX_LEAVES_COUNT) {
      throw new Error('Invalid parent count: ' + message.parents.length);
    }
  ...
  async findPastLeaves(leaves, timestamp) {
    const valid = [];
    const candidateHashes = new Set();

    for (const leaf of leaves) {
      if (leaf.content.timestamp <= timestamp) {
        valid.push(leaf);
        continue;
      }
      for (const parentHash of leaf.parents) {
        candidateHashes.add(parentHash.toString('hex'));
      }
    }

    // Just ignore invalid leaves to save resources
    if (valid.length !== 0) {
      return valid;
    }

    // Try candidates
    const hashes = Array.from(candidateHashes.values()).map((hash) => {
      return Buffer.from(hash, 'hex');
    });

    const candidates = await this.getMessages(hashes);
    return await this.findPastLeaves(candidates, timestamp);
  }
