...
export default class Channel {
  ...
  async post(body, identity) {
    ...
    this.debug('posted message.hash=%s', message.hash.toString('hex'));
  ...
  async receive(message) {
    ...
    this.debug('received message.hash=%s', message.hash.toString('hex'));
  }
  ...
  async query(cursor, isBackward, limit) {
    if (cursor.hash) {
      this.debug('got query cursor.hash=%s isBackward=%j limit=%d',
        cursor.hash.toString('hex'), isBackward, limit);
    } else {
  ...
  async bulk(hashes) {
    this.debug('got bulk hashes.length=%d', hashes.length);
    hashes = hashes.slice(0, MAX_BULK);

    const maybeMessages = await this.storage.getMessages(this.id, hashes);

    return {
      messages: maybeMessages.filter((message) => {
        return !!message;
      }),
  ...
  async sync(remote, isFull) {
    ...
    for (;;) {
      ...
      if (response.abbreviatedMessages.length > MAX_QUERY_LIMIT) {
      ...
      for (const abbr of response.abbreviatedMessages) {
        ...
        for (const { resolved, hash } of parents) {
          ...
          if (isFull) {
            throw new Error(`Missing parent: ${hash.toString('hex')} ` +
              'during full sync, aborting');
          }
      ...
      while (known.length !== 0) {
        ...
        for (const message of messages) {
          // Check that we requested this message
          const hexHash = message.hash.toString('hex');
          if (!expected.has(hexHash)) {
            throw new Error('Unexpected bulk message with hash: ' + hexHash);
          }
          expected.delete(hexHash);

          // Receive the message
          await this.receive(message);

          // Found the parent!
          unresolvedParents.delete(hexHash);
        }
  ...
  debug(format, ...args) {
    ...
    debug('id=[%s] ' + format, ...[ this.id.toString('hex') ].concat(args));
  }
}
