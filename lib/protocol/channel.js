...
import { now, WaitList } from '../utils';
...
export default class Channel {
  constructor(name, publicKey, options = {}) {
    ...
    this.lru = {
      hash: new LRU({ maxSize: this.options.maxLRUSize }),
      height: new LRU({ maxSize: this.options.maxLRUSize }),
    };
    ...
    this.messageWaitList = {
      incoming: new WaitList(),
      outgoing: new WaitList(),
    };
  }
  ...
  static async create(identity, name, options) {
    ...
    await channel.storage.addMessage({
      channelId: channel.id,
      hash: root.hash,
      parents: root.parents,
      height: root.height,
      data: root.serializeData(),
    });
    channel.lru.height.clear();
    channel.root = root;
  ...
  waitForIncomingMessage() {
    return this.messageWaitList.incoming.wait(null);
  }

  waitForOutgoingMessage() {
    return this.messageWaitList.outgoing.wait(null);
  }
  ...
  async post(body, identity) {
    ...
    this.lru.height.clear();
    this.debug('posted message.hash=%s', message.debugHash);

    this.messageWaitList.outgoing.resolve(null, message);
  ...
  async receive(message) {
    ...
    this.messageWaitList.incoming.resolve(null, message);
  }
  ...
  async getMessagesAtOffset(offset, limit = 1) {
    const serialized = await this.storage.getMessagesAtOffset(
      this.id, offset, limit);
    const messages = serialized.map((data) => {
  ...
  async getEncryptedLeaves() {
    ...
    return leaves.map((serialized) => {
      return Message.deserializeData(serialized);
    })
  }
  ...
  async getMessages(hashes) {
    const messages = hashes.map((hash) => {
      return this.lru.hash.get(hash.toString('hex'));
    });

    const missing = messages.map((message, i) => {
      if (message) {
        return false;
      }
    ...
    this.debug('got %d messages from LRU, requesting %d from storage',
      messages.length - missing.length,
      missing.length);

    const storageMessages = await this.storage.getMessages(
      this.id,
      missing.map((entry) => entry.hash));

    for (const [ i, serialized ] of storageMessages.entries()) {
      if (!serialized) {
        continue;
      }

      const message = Message.deserializeData(serialized);
      message.decrypt(this);
      messages[missing[i].index] = message;

      // Update LRU
      this.lru.hash.set(message.hash.toString('hex'), message);
    }
  ...
  async query(cursor, isBackward, limit) {
    ...
    return {
      abbreviatedMessages: full.messages.map((serialized) => {
        const message = Message.deserializeData(serialized);
        return {
          parents: message.parents,
          hash: message.hash,
...
Channel.MAX_LRU_SIZE = MAX_LRU_SIZE;
