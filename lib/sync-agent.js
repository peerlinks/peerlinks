...
export default class SyncAgent {
  ...
  async receiveEmptyResponse(seq) {
    ...
    this.waitList.resolve(`${entry.type}/${seq}`, null);
  }
  ...
  async receiveQueryResponse(seq, response) {
    ...
    this.waitList.resolve(`query/${seq}`, response);
  }
  ...
  async receiveBulkResponse(seq, response) {
    ...
    this.waitList.resolve(`bulk/${seq}`, response);
  }
  ...
  debug(fmt, ...args) {
    debug('peer.id=%s channel.id=%s ' + fmt,
      ...[ this.peerDebugId, this.channel.debugId ].concat(args));
  }
}
